...
partial class Program
{
        ...
        string? firstEncoding, bool downloadDanmaku, BBDownDanmakuFormat[] downloadDanmakuFormats, string input, string savePathFormat, string lang, string aidOri, string apiType, DownloadTask? relatedTask = null)
    {
        ...
        try
        {
            ...
            //调用解析
            ParsedResult parsedResult = await ExtractTracksAsync(aidOri, p.aid, p.cid, p.epid, myOption.UseTvApi, myOption.UseIntlApi, myOption.UseAppApi, myOption.UseTrial, firstEncoding);
            ...
            else if (parsedResult.Clips.Any() && parsedResult.Dfns.Any())   //flv
            {
                ...
                if (myOption.Interactive && !flag && !selected)
                {
                    ...
                    //重新解析
                    parsedResult.VideoTracks.Clear();
                    parsedResult = await ExtractTracksAsync(aidOri, p.aid, p.cid, p.epid, myOption.UseTvApi, myOption.UseIntlApi, myOption.UseAppApi, myOption.UseTrial, firstEncoding, dfns[vIndex]);
